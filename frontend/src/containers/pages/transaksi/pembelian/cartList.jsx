...
const CartList = (props) => {
    ...
    useEffect(() => {
        getTransPembelianJoin().then((data) => {
            ...
            if (dataFaktur.filter(({ statusPembelian }) => statusPembelian == 'onProcess').length == 0) {
                ...
                let fakturLama = dataFaktur.find(({ statusPembelian }) => statusPembelian == 'onProcess')
                let dataPembelian = arrayFilterPembelian.find(({ faktur }) => faktur == fakturLama.faktur)
    ...
    const handleHapusTransaksi = (val, val2, val3) => {
        Toast.fire({
            ...
            if (result.isConfirmed) {
                hapusTransaksiPembelian(val)
                axios.put(`/barangTerjual/${val2}/${val3}`)
                setRefresh(!refresh)
                Toast.fire(
                    'Terhapus!',
                    'Data Pembelian Sudah Terhapus.',
                    'success'
                )
            } else if (
                        ...
                        <tr key={ index }>
                            ...
                            <td>{ val.barang.nama }</td>
                            {
                                (display && val.id == idEdit)
                                    ...
                                    <>
                                        <td>
                                            <input type="text" className="form-control" id="jumlah" value={ jumlah }
                                                onChange={ (e) => setJumlah(e.target.value) } />
                                        </td>
                                        <td>
                                            <input type="text" className="form-control" id="hargaBeli" value={ editHargaBeli }
                                                onChange={ (e) => setEditHargaBeli(e.target.value) } />
                                        </td>
                                        <td>
                                            <input type="text" className="form-control" id="hargaBeli" value={ editHargaJual }
                            ...
                            <td>
                                {
                                    (display && val.id == idEdit)
                                        ...
                                        :
                                        <>
                                            <button className="btn btn-warning bx bx-edit-alt text-black-50" onClick={ () => handleEditJumlah(val.id, val.jumlah, val.harga, val.barang.hargaJual) } />
                                            <button className="bx bx-trash btn btn-danger " onClick={ () => handleHapusTransaksi(val.id, val.barangId, val.jumlah) } />
                                        </>
                                }
                            </td>
                        </tr>
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        handleFakturPembelian: (faktur) => dispatch({ type: ActionType.SET_FAKTUR_PEMBELIAN, index: faktur })
    }
}
