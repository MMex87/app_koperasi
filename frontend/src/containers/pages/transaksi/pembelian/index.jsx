  ...
  render() {
    ...
    const handlecart = async (e) => {
      ...
      if (barangKondisi != undefined) {
        barangIdLokal = this.state.barang.find(({ nama, supplier }) => nama == this.props.namaBarang && supplier.nama == this.props.supplier).id
      }
    ...
    const simpan = () => {
      let data = this.state.transaksi.filter(({ statusPembelian }) => statusPembelian == 'onProcess')
      Toast.fire({
                  ...
                  <form className="row g-3" onSubmit={ handlecart }>
                    <div className="col-md-6">
                      ...
                      <input
                        ...
                        autoComplete="false"
                        onChange={ (e) => this.props.handleSupplier(e.target.value) }
                        onClick={ () => this.setState({ displaySupplier: !this.state.displaySupplier }) }
                      />
                      { this.state.displaySupplier && (
                        <div className="flex-container flex-column pos-rel bodyAutoComplate">
                          <ul className="list-group list-group-flush">
                            { this.state.suppliers
                              .filter(({ nama }) => nama.indexOf(this.props.supplier) > -1)
                              .map((v, i) => (
                                <li
                                  key={ i }
                                  onClick={ () => {
              ...
              <div className="card">
                <div className="card-body">
                  ...
                  <div className='d-flex justify-content-end'>
                    <button className='btn btn-success' onClick={ simpan }>Simpan</button>
                  </div>
                </div>
              </div>
            </div>
